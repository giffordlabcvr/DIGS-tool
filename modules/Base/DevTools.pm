...
#***************************************************************************
# Subroutine:  Summarizes a perl module
# Description: prints the contents of a hash, including other arrays and
#              hashes.
# Arguments:   $hash_ref:  reference to the hash being printed
#              $formatting: additional formatting for recursive fxn call
#***************************************************************************
sub summarize_module {
...
sub print_web_array {
	...
	# don't show this line if it's a recursive call
	unless ($formatting) {
		print "<BR>&nbsp;#~o~ ~o~ ~o~ ~o~ ~o~ ~o~# Showing Array Contents \n";
	}

	foreach my $item(@$array_ref) {
		
		if (ref($item) eq 'ARRAY') { 
			print "<BR>&nbsp; Array in array:";
			$self->print_array($item, "<BR>");
		}
		elsif (ref($item) eq 'HASH' ) {
			print "<BR>&nbsp; Hash in array:";
			$self->print_hash($item, "<BR>");
		}	
		else {
			chomp $item;
			if ($formatting) { print "<BR>&nbsp;$formatting $item"; }
			else             { print "<BR>&nbsp;$item"; }
		}
	}
